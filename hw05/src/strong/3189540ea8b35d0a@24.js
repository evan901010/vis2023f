...
function _group_14(FileAttachment){return(
FileAttachment("Group_14@1.json").json()
)}

function _randomMemberScore(group_14){return(
() => {
  group_14.children[0].children.forEach((element, index) => {
...
function _strong(randomMemberScore,d3,group_14,drag,url_list,invalidation)
{
  // 指定圖表的尺寸。
  const width = 2000;
  const height = 400;
  ...
  // 計算圖形並啟動力模擬。
  const root = d3.hierarchy(group_14);
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["Group_14@1.json", {url: new URL("../Group_14.json", import.meta.url), mimeType: "application/json", toString}]
  ]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], _1);
  main.variable(observer("group_14")).define("group_14", ["FileAttachment"], _group_14);
  main.variable(observer("randomMemberScore")).define("randomMemberScore", ["group_14"], _randomMemberScore);
  main.variable(observer("url_list")).define("url_list", _url_list);
  main.variable(observer("drag")).define("drag", ["d3"], _drag);
  main.variable(observer("strong")).define("strong", ["randomMemberScore","d3","group_14","drag","url_list","invalidation"], _strong);
  return main;
}
